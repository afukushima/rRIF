...
iTable <- function(rRIF, top = 30) {
  ...
  if (top == "all" || top == "ALL")
    ...
    res <- utils::head(res[base::order(abs(res$RIF1), decreasing = TRUE),], top)
  DT::datatable(res)
}
...
##' This function plots the coexpression of the specified gene with each of the DE.
##' 
##' Modified Hudson::diffCoexPIF.plot()
##' (https://github.com/kevinrue/HudsonRIF)
##' 
##' @title Co-expression relationsips between the specified TF and the DE 
##'
##' @param rRIF "rRIF" class object
##' @param reg.gene The regulatory gene/probeset to plot co-expression values
##' @param pch.max A scaling factor defining the size of the data point associated with the maximal PIF value
##' @export
##' @examples
##' data(ToniData)
##' data(ToniData.DEGs)
...
diffCoexPIF.plot <- function(rRIF, reg.gene, pch.max = 0) {
  ...
  graphics::plot(x = rRIF$rBij[reg.gene, ], y = rRIF$rAij[reg.gene, ], xlim = c(-1, 1),
       ylim = c(-1, 1), cex = PIF.DE, xlab = rRIF$conds$B,
       ylab = rRIF$conds$A, pch = 20, main = reg.gene, col = "red")
...
paramCheck.diffCoexPIF.plot <- function(rRIF, probe, pch.max) {
  ...
  if(class(rRIF)[1] != "rRIF"){
    stop("\"rRIF\" (", class(rRIF),") is not an rRIF object.", call.=F)
  }
  # Note: $eSet, $classCol, $regulator.list, EiAB, Ai, dEi, dCij not necessary
  if(any(!c("target.factor", "DEGs", "PIFi", "rAij", "rBij", "RIF1", "RIF2") %in% names(rRIF))){
    stop("\"rRIF\" (", names(rRIF),") is not a valid rRIF object.
         One item of $target.factor, $DEGs, $PIFi, $rAij, $rBij or $RIF1/2 is missing.", call.=F)
  }
